...
<body>
	...
	<div class="container">
		...
		<div class="row">
			<form name="frm_movie" action="mainModify.wow" method="post" enctype="multipart/form-data">
				...
				<table class="table table-striped table-bordered">
					<tbody>
						<tr>
							<th>영화명</th>
							<td>
								<input type="text" name="movieNm" class="form-control input-sm" value="${movie.movieNm}">
								</td>
						</tr>
						<tr>
							<th>영화포스터</th>
							<td><img alt="" style="width: 20%" src="${pageContext.request.contextPath}/images/movie/${movie.movieImg}">
								<input type="hidden" name="movieImg" value="${movie.movieImg}">
								<input type="file" name="moviePoster" class="form-control input-sm">
							</td>
						</tr>
						<tr>
							<th>상영시간</th>
							<td><input type="number" name="showTm" class="form-control input-sm" value="${movie.showTm}">
								</td>
						</tr>
						<tr>
							<th>개봉일자</th>
							<td><input type="date" name="openDt" class="form-control input-sm" value="${movie.openDt}">
								</td>
						</tr>
						<tr>
							<th>시놉시스</th>
							<td><textarea rows="" cols="5" name="movieContent" class="form-control input-sm">${movie.movieContent}</textarea>
								</td>
						</tr>
						<tr>
							...
							<td>
								<div class="actor_area">
									<c:forEach items="${actors}" var="actor">
										<span>
											<input type="hidden" name="actorCd" value="${actor.actorCd}">${actor.actorNm}
											<button class="btn_actor_delete">삭제</button>
										</span>
									</c:forEach>
								</div>
						...
						<tr>
							...
							<td><div class="director_area">
									<c:forEach items="${directors}" var="director">
										<span>
											<input type="hidden" name="peopleCd" value="${director.peopleCd}">${director.peopleNm}
											<button class="btn_actor_delete">삭제</button>
										</span>
									</c:forEach>
								</div>
						...
						<tr>
							...
							<td><div class="company_area">
									<c:forEach items="${companys}" var="company">
										<span>
											<input type="hidden" name="companyCd" value="${company.companyCd}">${company.companyNm}
											<button class="btn_actor_delete">삭제</button>
										</span>
									</c:forEach>
								</div>
						...
						<tr>
							...
							<td>
								<c:forEach var="code" items="${cateList}">
									<input type="checkbox" name="commCd" value="${code.commCd}" 
									<c:forEach var="genre" items="${genres}">
										${genre.genreCd eq code.commCd ? "checked='checked'" : "" }
									</c:forEach>
									>${code.commNm}
								</c:forEach>
							</td>
						...
						<tr>
							<td colspan="2">
								<a href="mainView.wow?movieCd=${movie.movieCd}" class="btn btn-default btn-sm">
								  <span class="glyphicon glyphicon-list" aria-hidden="true"></span>
								  &nbsp;목록
								</a>
								<button type="submit" formaction="mainModify.wow" class="btn btn-primary btn-sm">
